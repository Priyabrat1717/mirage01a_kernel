...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
    ...
    /*if (doze_status == DOZE_ENABLED) {
		GTP_DEBUG("already in doze mode!\n");
		return 0;
	}
    if (device_may_wakeup(&ts->client->dev)) {
		ret = enable_irq_wake(ts->client->irq);
		if (ret)
		    GTP_DEBUG("%s: enable_irq_wake failed\n", __func__);
	}*/
    while(retry++ < 5)
    {
...
static int gtp_device_info_init(struct goodix_ts_data *data)
{
	...
	snprintf(ts_info, GTP_INFO_MAX_SIZE, "%s", data->product_id);
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    sprintf(fw_version, "%x", *version);
    strcat(product_id,link_flag);
    strcat(product_id,fw_version); 
    strncpy(ts->product_id,product_id,16);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
    {
       ...
       /* ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA,
			&opr_buf[0], 1);
        if (ret == SUCCESS) {
                    pr_err("%s IC Config Version: %d, 0x%02X)\n", __func__,opr_buf[0],opr_buf[0]);
          }*/
...
static int gtp_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	...
	//if (ev_data && ev_data->data && event == FB_EVENT_BLANK && ts) {
         if (ev_data && ev_data->data && event == FB_EARLY_EVENT_BLANK &&ts && ts->client){
		blank = ev_data->data;
