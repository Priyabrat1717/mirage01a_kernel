				...
				if (doze_buf[2] == 0xAA) {
					...
					dev_dbg(&ts->client->dev,
						"double click to light up the screen!");
	                                pr_err("%s(double click to light up the screen!)\n", __func__);
					doze_status = DOZE_WAKEUP;
					input_report_key(ts->input_dev,
								KEY_POWER, 1);
					input_sync(ts->input_dev);
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	if (doze_status == DOZE_ENABLED) {
		dev_dbg(&ts->client->dev, "already in doze mode!\n");
		return 0;
	}
	if (device_may_wakeup(&ts->client->dev)) {
		ret = enable_irq_wake(ts->client->irq);
		if (ret)
		    dev_err(&ts->client->dev, "%s: enable_irq_wake failed\n", __func__);
	}
	while (retry++ < GTP_I2C_RETRY_3) {
...
static ssize_t gtp_gesture_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
    struct goodix_ts_data *ts = dev_get_drvdata(dev);
...
static ssize_t gtp_gesture_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
    struct goodix_ts_data *ts = dev_get_drvdata(dev);
...
static DEVICE_ATTR(control_double_tap, (S_IRUGO | S_IWUSR | S_IWGRP),
			gtp_gesture_show,
			gtp_gesture_store);
...
static struct attribute *gtp_attrs[] = {
    ...
    &dev_attr_control_double_tap.attr,
	NULL
};
...
static ssize_t class_gtp_fw_info_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return snprintf(buf, GTP_INFO_MAX_SIZE, "%s\n", fw_info);
}
...
static CLASS_ATTR(fw_info, S_IRUSR, class_gtp_fw_info_show, NULL);
...
static int gtp_device_info_init(struct goodix_ts_data *data)
{
    ...
    fw_info = devm_kzalloc(&client->dev, GTP_INFO_MAX_SIZE, GFP_KERNEL);
	if (!fw_info) {
		dev_err(&client->dev, "Out of memory!\n");
		return -ENOMEM;
	}
	...
	if (rc < 0) {
        ...
        rc = class_create_file(touchscreen_class, &class_attr_fw_info);
	if (rc < 0) {
		dev_err(&client->dev, "%s class_create_file failed!\n", __func__);
		class_destroy(touchscreen_class);
		return rc;
    ...
    snprintf(fw_info, GTP_INFO_MAX_SIZE, "%s", pdata->fw_ver);
			   ...
			   const struct i2c_device_id *id)
{
    ...
    sprintf(fw_version, "%x", version_info);
    strncpy(ts->pdata->fw_ver,fw_version,strlen(fw_version)+1);
