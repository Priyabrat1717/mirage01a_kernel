...
int smscore_ts_file_write(void * dev, unsigned char* data, unsigned int size)
{
	struct smscore_device_t *coredev = dev;
	mm_segment_t oldfs;
	int ret;
	...
	ret = vfs_write(coredev->f, data, size, &coredev->f->f_pos);
					 ...
					 void *buffer, size_t size)
{
	...
	g_fw_checkSum = calc_checksum+1;
	while ( (g_fw_checkSum != calc_checksum) && 
			(DownloadAttempts < MAX_FW_DOWNLOAD_ATTEMPTS) )
	{
		...
		if (calc_checksum == g_fw_checkSum)
		{
...
static int smscore_load_firmware_from_file(struct smscore_device_t *coredev,
	...
	sms_info("loading firmware:%s. static buffer=0x%p", fw_filename, sms_static_firmware);
...
#ifdef SMS_MULTI_FW_SUPPORTED
	if ((mode == standby_mode) && (standby_mode != 0xff))
	{
		struct SmsMsgHdr_S *msg;
		...
		if (rc < 0)
			...
			{
			case SMSHOSTLIB_DEVMD_DVBT:
				standby_mode = SMSHOSTLIB_DEVMD_DVBT2;
				break;

			case SMSHOSTLIB_DEVMD_DVBT2:
				standby_mode = SMSHOSTLIB_DEVMD_DVBT;
				break;
...
static int smscore_load_standby_firmware(struct smscore_device_t *coredev, int mode)
{
	...
	firmware = (struct SmsFirmware_ST *)sms_static_firmware->data;
	FwSize = sms_static_firmware->fw_buf_size;
	...
	if (g_fw_checkSum != calc_checksum)
	{
...
int smscore_set_device_standby_mode(void* dev, int mode)
{
	...
	sms_debug("set device standby mode to %d", standby_mode);

	if (standby_mode != 0xff)
 		rc = smscore_load_standby_firmware(coredev, standby_mode);
...
void smscore_onresponse(void* dev, struct smscore_buffer_t *cb) 
{
	...
	if (rc < 0) {
		...
		case MSG_SMS_DATA_VALIDITY_RES:
		{
			...
			g_fw_checkSum = validity->msgData[0];
