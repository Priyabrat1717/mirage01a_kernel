...
int smschar_send_fw_file(struct smschar_device_t *dev,
	...
	/* free old buffer */
#ifdef SMS_FW_MEM_MALLOC_ONCE
	if (!sms_static_firmware || (FW_SIZE < up->fw_size)) {
		sms_err("static_firmware_mem:%p, size:%d, FW size:%d",sms_static_firmware, FW_SIZE, up->fw_size);
		
		return -ENOMEM;
	}
	memset(sms_static_firmware, 0 , ALIGN(FW_SIZE + sizeof(sms_static_firmware), SMS_ALLOC_ALIGNMENT));
#else
...
int smschar_register(void)
{
	...
	if (host_sms_reset_pin != 0xFFFFFFFF)
	{
		...
		sms_info("buffer 0x%p allocated. copy filename\n", sms_static_firmware);
#endif
